...
### 출력 인수
일반적으로 인수를 **함수 입력**이라고 해석한다. 인수를 출력으로 사용하는 함수에 어색함을 느낀다. 객체 지향 프로그래밍이 나오기 전에는 출력 인수가 불가피한 경우도 있었지만 객체 지향 언어가 나온 이후로는 출력 인수를 사용할 필요가 거의 없다. 이때는 `this`를 사용하면 된다.
...
```
 이 함수는 attribute 속성을 찾아 value로 설정한 후 성공하면 true를, 실패하면 false를 반환한다.

 ```java
 if(set("username", "unclebob")) ...
 ```

 이런 경우 무엇을 의미하는지 모호하다. 해결책은 **명령과 조회를 분리**하는 것이다.
 ...
 # 오류 코드보다 예외를 사용하라!
 ## try/catch 블록 뽑아내기
 try/catch 블록은 코드구조에 혼란을 일으키며, 정상 동작과 오류 처리 동작을 뒤섞는다.
 ...
 try/catch 문 안에서 정상 동작하는 함수와 오류 발생시 처리하는 함수를 따로 분리하도록 한다.

 <hr/>

 # 반복하지 마라!
 계속해서 반복되는 코드는 부모 클래스로 옮겨 중복을 없애도록 한다.

 <hr/>

 # 3장 정리 - 생각
 함수는 한 가지의 역할을 수행해야하며 추상화 수준의 단계 레벨에 맞게 잘 배치해야한다는 것을 강조한다. 또한 블록 내의 내용들은 함수로 표현하도록 해야한다. 

 여태 내가 짰던 코드들은 왠지 의식의 흐름대로만 짰던 것 같아 좀 더 생각해보고 깔끔하게 짤 수 있도록 노력해야겠다.
